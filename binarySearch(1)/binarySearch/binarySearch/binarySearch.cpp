...
int main()
{
	int nums[] = {3,3,3,4,5,6};
	printf("普通二分查找，找3在:%d\n", binarySearch(nums, 3, sizeof(nums) / sizeof(nums[0])));
	printf("左边界二分查找，找3在:%d\n", left_bound(nums, 3, sizeof(nums) / sizeof(nums[0])));
	printf("右边界二分查找，找3在:%d\n", right_bound(nums, 3, sizeof(nums) / sizeof(nums[0])));
}